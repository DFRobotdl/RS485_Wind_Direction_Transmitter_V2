...
uint16_t RS485_Wind_Direction_Transmitter_V2::CRC16_2(uint8_t* buf,
                                                      int16_t len) {
    uint16_t crc = 0xFFFF;
...
bool RS485_Wind_Direction_Transmitter_V2::readHoldingRegister(uint8_t Address,
                                                              uint16_t reg,
                                                              uint8_t* data) {
    uint8_t buf[7] = {
        0};  // Store the original data packet returned by the sensor
    uint8_t COM[8] = {0x00, 0x03, 0x00, 0x00,
                      0x00, 0x01, 0x00, 0x00};  // Read register command
    long curr = millis();                       // Determine timeout
    long curr1 = curr;                          // Determine timeout
    uint8_t ch = 0;                             // Store serial data temporarily
    COM[0] = Address;  // Refer to the communication protocol to add the
    ...
    while (1) {
        if (millis() - curr > 1000) {
            return false;  // Timeout if correct data is not read in 1000ms, and
                           // return —1
        }

        if (millis() - curr1 >
            100) {  // Re-transmit command for reading wind direction if the
                    // return command is not received in 100ms after sending
                    // last wind direction reading command.
            ...
            if (ch == Address) {  // Read and determine packet header
                ...
                if (readN(&ch, 1) == 1) {
                    if (ch == 0x03) {  // Read and determine packet header
                        ...
                        if (readN(&ch, 1) == 1) {
                            if (ch ==
                                0x02) {  // Read and determine packet header
                                buf[2] = ch;
                                if (readN(&buf[3], 4) == 4) {
                                    // DBUG(buf, 8);
                                    if (CRC16_2(buf, 5) ==
                                        (buf[5] * 256 +
                                         buf[6])) {  // Check data packet
                                        data[0] = buf[3];
                                        data[1] = buf[4];
                                        return true;
                                    }
...
bool RS485_Wind_Direction_Transmitter_V2::writeHoldingRegister(
    uint8_t Address,
    uint16_t reg,
    uint16_t parameter) {
    uint8_t COM[11] = {0x00, 0x10, 0x00, 0x00, 0x00, 0x01,
                       0x02, 0x00, 0x00, 0x00, 0x00};  // Write register command
    uint8_t buf[8] = {0};  // Store data returned by the sensor
    long curr = millis();  // Determine timeout
    long curr1 = curr;     // Determine timeout
    uint8_t ch = 0;        // Store serial data temporarily
    COM[0] = Address;  // Refer to the communication protocol to add complete
                       // command packet
    COM[2] = (reg / 0x100) % 0x100;
    COM[3] = reg % 0x100;
    ...
    while (1) {
        ...
        if (millis() - curr1 > 100) {
            _ser->write(COM, 11);  // Transmit command for reading wind
                                   // direction
            curr1 = millis();
        }
...
uint8_t RS485_Wind_Direction_Transmitter_V2::GetSlaveAddress() {
    uint8_t data[2] = {0};
    readHoldingRegister(0x00, 0x1000, data);
    return data[1];
}

uint8_t RS485_Wind_Direction_Transmitter_V2::GetWindDirection(uint8_t Address) {
    uint8_t data[2] = {0};
    readHoldingRegister(Address, 0x0001, data);
    return data[0] * 0x100 + data[1];
}

float RS485_Wind_Direction_Transmitter_V2::GetWindAngle(uint8_t Address) {
    uint8_t data[2] = {0};
    readHoldingRegister(Address, 0x0000, data);
    return (data[0] * 0x100 + data[1]) / 10.0;
}

bool RS485_Wind_Direction_Transmitter_V2::SetSlaveAddress(uint8_t Address1,
                                                          uint8_t Address2) {
